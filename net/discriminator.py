...
class patchGAN(nn.Module):
    def __init__(self,input_channel, K=64, n_layers=4):
        super(patchGAN, self).__init__()
        model = []
        model += [nn.Conv2d(input_channel, K, kernel_size=4, stride=2, padding=1), nn.LeakyReLU(0.2, inplace=True)]
        temp_k = K
        ...
        for i in range(1, n_layers):
            ...
            if temp_k <= 256:
                model += network.cNsN_K(temp_k,2,N=4,k=temp_k*2,padding=1,norm=nn.InstanceNorm2d,activation=nn.LeakyReLU(0.2,inplace=True))
                temp_k *= 2
            else:
                model += network.cNsN_K(temp_k,2,N=4,k=temp_k,padding=1,norm=nn.InstanceNorm2d,activation=nn.LeakyReLU(0.2,inplace=True))
